...
const demoDashboard: Dashboard = {
  ...
  work_objects: [
    {
      ...
      evidence: [
        {
          id: 'e-1',
          source_type: 'ClickUp',
          source_title: 'Hermes build task',
      ...
      plan_steps: [
        { id: 'p-1', title: 'Complete command-center foundation', owner_name: 'Hermes', status: 'in_progress', risk_class: 'read', position: 1, depends_on: [] },
        ...
        { id: 'p-3', title: 'Run the first @Hermes delegation', owner_name: 'Hermes', status: 'proposed', risk_class: 'reversible_write', position: 3, depends_on: ['p-1', 'p-2'] },
      ],
      approvals: [
        {
          id: 'a-1',
  ...
  activity: [],
  integrations: [
    { id: 'hermes', name: 'Hermes Agent', status: 'scaffolded', detail: 'Private agent API is ready to configure.' },
...
const App: React.FC = () => {
  ...
  const stageResearch = async () => {
    try {
      ...
      setSelected(await response.json() as WorkObject);
      setNotice('Hermes staged a source-grounded research-to-decision draft for review.');
    } catch {
      setNotice('Research staging is ready once the API is running. The production run will use Hermes, Cognee, ClickUp, and Otter within the selected scope.');
    }
  };
  ...
  const createWork = async (event: FormEvent) => {
    ...
    try {
      ...
      setNotice('New delegation staged. Hermes will first propose research scope and a plan.');
      await refresh();
    } catch {
        ...
        <aside className="hidden w-[250px] shrink-0 flex-col border-r border-white/[0.07] bg-[#0d1118]/95 px-4 py-5 lg:flex">
          <div className="mb-9 flex items-center gap-3 px-2">
            ...
            <div>
              ...
              <p className="text-[10px] font-medium uppercase tracking-[0.18em] text-violet-300">Chief of Staff</p>
            </div>
          </div>
              ...
              <section className="rounded-2xl border border-white/[0.08] bg-[#111721]/80 p-5 shadow-2xl shadow-black/10 xl:col-span-2">
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  {[
                    ...
                    ['Research run', 'The pilot needs a scope, evidence pack, decision brief, and plan review.', 'Hermes', 'fa-compass-drafting'],
